...
def update():
    for i in range(0, len(part_w_x)) : # water behavior
    ...
    for i in range(0, len(part_d_x)) : # dirt behavior
        ...
        elif get_pixel(part_d_x[i]-PIXEL_DIM, part_d_y[i]+PIXEL_DIM +1) == WHITE and get_pixel(part_d_x[i]-PIXEL_DIM, part_d_y[i]) == WHITE:
            part_d_x[i] -= PIXEL_DIM
        elif get_pixel(part_d_x[i]+PIXEL_DIM, part_d_y[i]+PIXEL_DIM +1) == WHITE and get_pixel(part_d_x[i]+PIXEL_DIM, part_d_y[i]) == WHITE:
            part_d_x[i] += PIXEL_DIM
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN :
            ...
            if event.key == pg.K_RIGHT :
                ...
                summon_particles(mouse_x-(mouse_x%PIXEL_DIM), mouse_y-(mouse_y%PIXEL_DIM), "water")

            if event.key == pg.K_LEFT :
                (mouse_x, mouse_y) = pg.mouse.get_pos()
                summon_particles(mouse_x-(mouse_x%PIXEL_DIM), mouse_y-(mouse_y%PIXEL_DIM), "dirt")
